...
void encryption()
{
	...
	printf("\nEnter the key for encryption (only uppercase alphabets): ");
	...
	{
		cipher_text[i] = (((plain_text[i] - 'A') + (key[i % len] - 'A')) % 26) + 'A';
	}
	cipher_text[i] = '\0';
...
void decryption()
{
	...
	printf("\nEnter the key for decryption (only uppercase alphabets): ");
	fgets(key, sizeof(key), stdin);
	key[strcspn(key, "\n")] = '\0';
	...
	{
		plain_text[i] = (((cipher_text[i] - 'A') - (key[i % len] - 'A') + 26) % 26) + 'A';
	}
	plain_text[i] = '\0';
